...
class OptimizeJsPro
{
	...
	public static function doInline($htmlSource)
	{
		...
		preg_match_all('#<script[^>]*(type|src)(|\s+)=(|\s+)[^>]*(>)(.*)</script>#Usmi', OptimizeCommon::cleanerHtmlSource($htmlSource, array('strip_content_between_conditional_comments')), $matchesSourcesFromTags, PREG_SET_ORDER);
		...
		if (! empty($matchesSourcesFromTags)) {
			...
			foreach ($matchesSourcesFromTags as $matchList) {
				...
				$chosenInlineJsMatches = (! empty($allPatterns) &&
				...
				preg_match_all('#src=(["\'])' . '(.*)' . '(["\'])#Usmi', $matchedTag, $outputMatches);
				...
				$jsContent = trim(OptimizeJs::maybeAlterContentForJsFile($jsContent, false));
