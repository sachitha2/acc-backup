...
class Positions
{
	...
	public static function doChanges($htmlSource)
	{
		...
		$cleanStylesIds = wp_cache_get('wpacu_styles_positions_handle_ids');
	...
	public function markStylesToLoadInNewPosition($unloadedList)
	{
		...
		foreach ($assetsPositions['styles'] as $styleHandle => $styleNewPosition) {
			if (isset(Main::instance()->wpAllStyles['registered'][$styleHandle]->src)) {
				...
				if (isset($wpStylesRegistered[$styleHandle])) {
					...
					wp_cache_set('wpacu_maybe_optimize_it_css_'.$styleHandle, $optimizeValues);
		...
		wp_cache_set('wpacu_styles_positions_tags', $stylesNewPositionsTags);
		wp_cache_set('wpacu_styles_positions_handle_ids', $stylesNewPositionsHandleIds);
	}
...
JS;
	...
	{
		$stylesPositionsTags = wp_cache_get('wpacu_styles_positions_tags');
				...
				if ($scriptHandle === 'jquery-core') {
					...
					if ($scriptNewPositionInt === 1) {
						add_action('wp_body_open', static function() {
							...
							wp_cache_add('wpacu_jquery_script_tag', $htmljQueryScriptTag);
							wp_cache_add('wpacu_jquery_migrate_script_tag', $htmljQueryMigrateScriptTag);
	...
	public function maybeMovejQueryAfterBodyTag($htmlSource)
	{
		if ((strpos($htmlSource, self::DEL_MAYBE_PLACE_JQUERY_RIGHT_AFTER_BODY) !== false)
		    && ($jQueryScriptTag = wp_cache_get('wpacu_jquery_script_tag'))) {
			...
			if ($jqueryMigrateNotLoaded) {
				$jQueryMigrateScriptTag = wp_cache_get('wpacu_jquery_migrate_script_tag');
