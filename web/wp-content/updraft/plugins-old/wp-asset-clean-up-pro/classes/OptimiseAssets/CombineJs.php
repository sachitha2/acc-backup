		...
		if (empty($finalCacheList)) {
			...
			*/
			...
			if ($minifyJsInlineTagsIsNotEnabled) {
				$wpacuDocumentForJS = new \DOMDocument();
				...
				$wpacuDocumentForJS->loadHTML( $htmlSourceAlt );
			} else {
				$wpacuDocumentForJS = OptimizeCommon::getDomLoadedTag($htmlSource, 'combineJs');
			}
			...
			foreach ( array( 'head', 'body' ) as $docLocationScript ) {
				...
				$docLocationElements = $wpacuDocumentForJS->getElementsByTagName($docLocationScript)->item(0);
				...
				if ($docLocationScript && Main::instance()->settings['combine_loaded_js_defer_body']) {
					wp_cache_set('wpacu_html_dom_body_tag_for_js', $docLocationElements);
				}
...
HTML;
		...
		if (isset($finalCacheList['body']) && (! empty($finalCacheList['body'])) && Main::instance()->settings['combine_loaded_js_defer_body']) {
			// CACHE RE-BUILT
			if ($isDeferAppliedOnBodyCombineGroupNo > 0 && $wpacuDocumentForJS = wp_cache_get('wpacu_html_dom_body_tag_for_js')) {
				...
				$scriptTags = $wpacuDocumentForJS->getElementsByTagName('script');
			} else {
				// FROM THE CACHE
