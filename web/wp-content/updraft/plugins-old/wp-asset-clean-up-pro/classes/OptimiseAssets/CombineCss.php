	...
	public static function doCombine($htmlSource)
	{
		...
		if (empty($storageJsonContents)) {
			...
			*/
			...
			$documentForCSS = OptimizeCommon::getDomLoadedTag($htmlSource, 'combineCss');
				...
				foreach ($linkTags as $tagObject) {
					...
					if (isset($linkAttributes['rel'], $linkAttributes['href']) && $linkAttributes['href']) {
						...
						if (isset($linkAttributes['data-wpacu-style-handle']) &&
						    ...
						    wp_cache_get($linkAttributes['data-wpacu-style-handle'], 'wpacu_basic_preload_handles')) {
			...
			foreach ($storageJsonContents as $docLocationTag => $mediaValues) {
				...
				foreach ($mediaValues as $mediaValue => $storageJsonContentLocation) {
					...
					if (strpos($mediaValue, 'wpacu_preload_async_') === 0) {
						...
						$finalCssTag = <<<HTML
<link rel='preload' as='style' onload="this.rel='stylesheet'" data-wpacu-preload-it-async='1' id='wpacu-combined-css-{$docLocationTag}-{$groupLocation}-preload-it-async' href='{$finalTagUrl}' type='text/css' media='{$mediaValueClean}' />
HTML;
