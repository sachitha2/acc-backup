...
class OptimizeCommon
{
	...
	{
		$cacheIt = false; // default

		if (! ($domTag = wp_cache_get('wpacu_html_dom_tag'))) { // Not cached?
			...
			if ($for === 'removeMetaGenerators') {
				$cleanerDomRegEx = '@<(noscript|style|script)[^>]*?>.*?</\\1>@si';
			}
			// [/HTML CleanUp]
			...
			if ($cacheIt) {
				wp_cache_set( 'wpacu_html_dom_tag', $domTag );
			}
		}
