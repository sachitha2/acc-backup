...
foreach ($data['all']['styles'] as $obj) {
	...
	*/
	...
	// [wpacu_pro]
	$isUnloadRegExMatch        = isset($data['unloads_regex_matches']['styles']) && in_array($data['row']['obj']->handle, $data['unloads_regex_matches']['styles']);
	$isLoadExceptionRegExMatch = isset($data['load_exceptions_regex_matches']['styles']) && in_array($data['row']['obj']->handle, $data['load_exceptions_regex_matches']['styles']);
	// [/wpacu_pro]
	...
	if (! $isLoadException && ($data['row']['is_group_unloaded'] || $isUnloadRegExMatch)) {
		if ($isUnloadRegExMatch) {
			$data['row']['is_regex_unload_match'] = true;
		}
