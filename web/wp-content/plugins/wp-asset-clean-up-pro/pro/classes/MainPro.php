...
class MainPro
{
		    ...
		    add_action ($current_screen->taxonomy . '_edit_form_fields', static function ($tag) {
		        if (! Main::instance()->settings['dashboard_show']) {
                    ...
                    <tr class="form-field">
                        ...
                        <td><?php echo sprintf(__('"Manage in the Dashboard?" is not enabled in the plugin\'s "%sSettings%s", thus, the list is not available.', 'wp-asset-clean-up'), '<a href="admin.php?page=wpassetcleanup_settings">', '</a>'); ?></td>
                    </tr>
                    <?php
                    return;
                }
	...
	public function getAssetFileSize($obj, $format = 'for_print')
	{
	    if (isset($obj->src) && $obj->src) {
		    ...
		    $isRelInternalPath = strpos($src, '/') === 0 && strpos($src, '//') !== 0;
