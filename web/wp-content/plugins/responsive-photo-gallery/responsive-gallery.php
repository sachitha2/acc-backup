...
add_action('in_admin_header','admin_content_rpg'); 
...
function as_settings_link_rpg($links) {
    ...
    $as_settings_link2= '<a href="edit.php?post_type=responsive-gallery">Settings</a>';
    array_unshift($links, $as_settings_link1, $as_settings_link2);
    return $links;
}

// Review Notice Box
add_action("admin_notices","review_admin_notice_rpg");
function review_admin_notice_rpg(){
    $screen = get_current_screen();
    if($screen->post_type == "responsive-gallery"){
        echo '<div class="notice notice-success is-dismissible review-notice">
		<p>Thanks for installing and using RESPONSIVE PHOTO GALLERY plugin. If you love our plugin and plugin is really work for you.
		Then please share your feedback about this plugin. Your feedback will be helpful to make plugin more error free.</p>
		<p><a href="https://wordpress.org/support/plugin/responsive-photo-gallery/reviews/?filter=5" target="_blank" name="review" id="review" class="button button-primary">Review & Rate</a></p>
		</div>';
		?>
		<style>
		.review-notice{
